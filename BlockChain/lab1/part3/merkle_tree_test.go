...
func createData(size, digit int) [][]byte {
	data := make([][]byte, size)
	for i := 0; i < size; i++ {
		idx := fmt.Sprintf("%0*d", digit, i)
		data[i] = []byte("data" + idx)
	}
	return data
}

func benchmarkTestNewMerkelTree(size, digit int, b *testing.B) {
	data := createData(size, digit)
	// hash := sha256.New()
	// hash.Write(data[3224%size])
	for n := 0; n < b.N; n++ {
		NewMerkleTree(data)
	}
}

func BenchmarkTestNewMerkelTree100(b *testing.B) {
	benchmarkTestNewMerkelTree(100, 3, b)
}

func BenchmarkTestNewMerkelTree1000(b *testing.B) {
	benchmarkTestNewMerkelTree(1000, 4, b)
}

func BenchmarkTestNewMerkelTree10000(b *testing.B) {
	benchmarkTestNewMerkelTree(10000, 5, b)
}

func benchmarkTestVerifyProof(size, digit int, b *testing.B) {
	data := createData(size, digit)
	mt := NewMerkleTree(data)
	hash := sha256.New()
	hash.Write(data[3224%size])
	mp := MerkleProof{}
	mp.proof, mp.index, _ = mt.MakeMerkleProof(hash.Sum(nil))
	for n := 0; n < b.N; n++ {
		VerifyProof(mt.MerkleRootHash(), hash.Sum(nil), mp)
	}
}

func BenchmarkTestVerifyProof100(b *testing.B) {
	benchmarkTestVerifyProof(100, 3, b)
}

func BenchmarkTestVerifyProof1000(b *testing.B) {
	benchmarkTestVerifyProof(1000, 4, b)
}

func BenchmarkTestVerifyProof10000(b *testing.B) {
	benchmarkTestVerifyProof(10000, 5, b)
}
=======
>>>>>>> 5c06b006eddeb9b9814aaa40bb36fc4ef6af0707
