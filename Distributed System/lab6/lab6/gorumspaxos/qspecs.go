...
func (qs PaxosQSpec) AcceptQF(accMsg *pb.AcceptMsg, replies map[uint32]*pb.LearnMsg) (*pb.LearnMsg, bool) {
	...
	for _, v := range replies {
		...
		if v.Val.ClientID != accMsg.Val.ClientID || v.Val.ClientSeq != accMsg.Val.ClientSeq || v.Val.Tx.Op != accMsg.Val.Tx.Op || v.Val.Tx.Amount != accMsg.Val.Tx.Amount {
			return nil, false
		}
...
func (qs PaxosQSpec) ClientHandleQF(in *pb.Value, replies map[uint32]*pb.Response) (*pb.Response, bool) {
	...
	for _, v := range replies {
		if in.ClientID == v.ClientID && in.ClientSeq == v.ClientSeq && int32(in.AccountNumber) == int32(v.TxResult.AccountNumber) {
			validData++
			txResult = v.TxResult
		}
