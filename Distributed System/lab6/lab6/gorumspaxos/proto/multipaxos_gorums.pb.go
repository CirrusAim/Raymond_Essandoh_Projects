...
type QuorumSpec interface {
	...
	// be used by the quorum function. If the in parameter is not needed
	// you should implement your quorum function with '_ *LearnMsg'.
	CommitQF(in *LearnMsg, replies map[uint32]*Empty) (*Empty, bool)
...
// with the same argument in, and returns a combined result.
func (c *Configuration) Commit(ctx context.Context, in *LearnMsg) (resp *Empty, err error) {
	cd := gorums.QuorumCallData{
		Message: in,
	...
	cd.QuorumFunction = func(req protoreflect.ProtoMessage, replies map[uint32]protoreflect.ProtoMessage) (protoreflect.ProtoMessage, bool) {
		...
		return c.qspec.CommitQF(req.(*LearnMsg), r)
	}
...
type MultiPaxos interface {
	...
	Commit(ctx gorums.ServerCtx, request *LearnMsg) (response *Empty, err error)
...
func RegisterMultiPaxosServer(srv *gorums.Server, impl MultiPaxos) {
	...
	srv.RegisterHandler("proto.MultiPaxos.Commit", func(ctx gorums.ServerCtx, in *gorums.Message, finished chan<- *gorums.Message) {
		req := in.Message.(*LearnMsg)
		defer ctx.Release()
		resp, err := impl.Commit(ctx, req)
		gorums.SendMessage(ctx, finished, gorums.WrapMessage(in.Metadata, resp, err))
	})
