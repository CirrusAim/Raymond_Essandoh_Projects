...
func (p *Proposer) performAccept() error {
	...
	for p.acceptMsgQueue.Len() > 0 {
		fmt.Println("Called performAccept acceptMsgQueue")
...
func (p *Proposer) performCommit() error {
	...
	p.Lock()
	for msg := p.clientRequestQueue.Front(); msg != nil; msg = msg.Next() {
		val, _ := msg.Value.(*pb.Value)
		if val != nil && p.learnMsg.Val != nil && val.AccountNumber == p.learnMsg.Val.AccountNumber {
			p.clientRequestQueue.Remove(msg)
			break
		}
	}
